from cryptography.hazmat.primitives.asymmetric import dh
from cryptography.hazmat.primitives import serialization
from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC
from cryptography.hazmat.primitives import hashes

# ----------------------------------------
# 1. Tạo cặp khóa Diffie-Hellman cho client
# ----------------------------------------
def generate_client_key_pair(parameters):
    private_key = parameters.generate_private_key()
    public_key = private_key.public_key()
    return private_key, public_key

# ----------------------------------------
# 2. Tạo khóa bí mật chia sẻ giữa client và server
# ----------------------------------------
def derive_shared_secret(private_key, server_public_key):
    shared_key = private_key.exchange(server_public_key)
    return shared_key

# ----------------------------------------
# 3. Chương trình chính
# ----------------------------------------
def main():
    # Load public key của server từ file
    with open("server_public_key.pem", "rb") as f:
        server_public_key = serialization.load_pem_public_key(f.read())

    # Lấy tham số DH từ public key
    parameters = server_public_key.parameters()

    # Sinh khóa của client
    private_key, public_key = generate_client_key_pair(parameters)

    # Tính shared secret
    shared_secret = derive_shared_secret(private_key, server_public_key)

    print("✅ Shared Secret:", shared_secret.hex())

# ----------------------------------------
# 4. Gọi hàm main khi chạy trực tiếp
# ----------------------------------------
if __name__ == "__main__":
    main()
